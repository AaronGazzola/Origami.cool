...
const OrderScreen = ({ match }) => {
	...
	const { order, loading: getOrderLoading } = useSelector(
		state => state.getOrder
	);
	const { order: canceledOrder } = useSelector(state => state.cancelOrder);
	...
	return (
		<>
			...
			>
				...
				{!getOrderLoading && order?.isCanceled && (
							...
							>
								...
								{getOrderLoading ? (
									...
									>
										<Typography
											variant='h5'
											className={contentClasses.subTitle}
										>
											{order?.user.name}
										</Typography>
										...
										{order?.address &&
											Object.keys(order?.address).map(key =>
												order?.address[key] && key === 'state' ? (
													<Typography
														key={key}
														className={contentClasses.address}
													>
														{order?.address.state}&nbsp;&nbsp;&nbsp;
							...
							{getOrderLoading ? (
								...
								>
									...
									{order?.isDelivered ? (
										<>
											<Typography className={classes.content}>
												Your order has been sent by regular parcel post and
												should arrive within 3-10 business days.
											</Typography>
											<Typography className={classes.content}>
												Date sent:{' '}
